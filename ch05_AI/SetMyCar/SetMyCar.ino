...
int car_speed_tmp = map(20,0,100,0,255);
...
static esp_err_t cmd_handler(httpd_req_t *req){
  ...
  if(!strcmp(variable, "forward")) {
    ...
    Serial.println("Left");
    car_go_left(car_speed, car_speed_tmp);
  }
  else if(!strcmp(variable, "right")) {
    Serial.println("Right");
    car_go_right(car_speed, car_speed_tmp);
  }
    ...
    // 회전 속도
    car_speed_tmp = map(20,0,100,0,255);
    ...
    // 회전 속도
    car_speed_tmp = map(40,0,100,0,255);
    Serial.println(car_speed);
  }
    ...
    // 회전 속도
    car_speed_tmp = map(60,0,100,0,255);
    Serial.println(car_speed);
  }
    ...
    // 회전 속도
    car_speed_tmp = map(80,0,100,0,255);
    Serial.println(car_speed);
  }
  else {
    res = -1;
...
// 전진하며 회전하는 메커니즘으로 변경
void car_go_left(int speed, int speed_tmp){
  //오른쪽모터
  digitalWrite(IN_11,HIGH);
  analogWrite(IN_12,speed);
  ...
  //왼쪽모터
  digitalWrite(IN_21,LOW);
  analogWrite(IN_22,speed_tmp);
}

void car_go_right(int speed, int speed_tmp){
  //오른쪽모터
  digitalWrite(IN_11,HIGH);
  analogWrite(IN_12,speed_tmp);
